...
img_dict = dict()
for f in img_files:
	f_noext = re.sub('\.[A-Za-z0-9]+$', '', f)
	ROI_files = [r for r in all_ROI_files if f_noext in r]
	if len(ROI_files) > 0:
		ROI_dict[f_noext] = ROI_files[0]#should only be one
		img_dict[f_noext] = f
...
for f in img_dict.keys():#will skip any image files without matching ROIs - e.g. excluded from analysis
	print("Processing image " + img_dict[f])
	img = IJ.openImage(imgdir + '/' + img_dict[f])#os.path.join not working - string vs list input?
	...
	for c,n in to_quantify.items():
		...
		results.save(ROIdir + "/fluorescence_quantification/" + img_dict[f] + "_" + n + "_quantification.csv")
	img.close()
	rm.reset()
	results.reset()
